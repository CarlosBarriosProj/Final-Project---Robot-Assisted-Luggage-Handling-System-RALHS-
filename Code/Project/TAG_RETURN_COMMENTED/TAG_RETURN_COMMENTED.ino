// CARLOS ANTONIO BARRIOS GUZMAN || FINAL PROJECT || 19/04/2026 || TAG RETURN (HOME POSITION)
...
// ===================== UWB SETUP =====================
#define DEVICE_ADDRESS "04:00:00:00:00:00:00:02"
...
if (currentShortAddress != nullptr) {
  ...
  Serial.println("Failed to get short address");
}
...
void newRange() {
  ...
  // ONLY WHEN THE CURRENT STATE AND THE DATA VALIDITY ALLOW IT.
  if (START_UP) {
    display.clearDisplay();
  ...
  static float buffer84[5] = {NAN, NAN, NAN, NAN, NAN};
  static float buffer85[5] = {NAN, NAN, NAN, NAN, NAN};
  static int index84 = 0;
  static int index85 = 0;
  static int count84 = 0;
  static int count85 = 0;

  static unsigned long lastUpdate = 0;
  static unsigned long lastSendTime = 0;

  static float latestAvg84 = NAN;
  static float latestAvg85 = NAN;
  ...
  // ONLY COMPUTE THE AVERAGE AFTER EACH BUFFER HAS 5 REAL SAMPLES
  if (count84 == 5) {
    latestAvg84 = 0.0f;
    for (int i = 0; i < 5; i++) {
      latestAvg84 += buffer84[i];
    }
    latestAvg84 /= 5.0f;
  } else {
    latestAvg84 = NAN;
  ...
  // HERE THE TAG IS ONLY ALLOWED TO SEND WHEN THE SYSTEM IS IN STATE 4
  // AND BOTH FILTERED DISTANCES ARE ALREADY VALID.
  if (NUMBER_TAPS == 4 &&
    ...
    // BEFORE STATE 4 THIS TAG DOES NOT SEND RETURN DISTANCES YET
    Serial.println("RETURN_TAG not enabled yet");
  }
}
